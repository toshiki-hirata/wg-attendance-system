      ...
      <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="flex flex-col">
          ...
          <div
            ...
            :class="{
              'flex items-center justify-center text-gray-500': !getResponse,
            }"
          >
            <template v-if="getResponse">
              <pre>{{ JSON.stringify(getResponse, null, 2) }}</pre>
            </template>
            <template v-else> Response will appear here </template>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="flex flex-col">
          ...
          <div
            class="mt-4 p-4 bg-gray-50 rounded border border-gray-200 flex-1 overflow-auto"
            :class="{
              'flex items-center justify-center text-gray-500': !postResponse,
            }"
          >
...
const handlePostRequest = async () => {
  try {
    const response = await axiosInstance.post<PostSampleResponse>(
      '/post',
      postData.value
    );
